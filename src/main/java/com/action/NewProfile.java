...
public class NewProfile extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (name != null && name != "" && password != null && password != "" && emailid != null && emailid != "" && sex != "" && sex != null) {
            	if (IntrusionDetector.isXSSAttack(name, password, emailid, sex)) {
            		Reporter.logAttack(request, "XSS Attack");
            		response.sendRedirect("index.jsp?msg=Attempt of XSS Attack");
            	} else {
            		String sql = "insert into user values('" + name + "','" + password + "','" + sex + "','"+ emailid + "',now())";
